...
if (isset($_POST['search'])) {
    extract($_POST);
    $allSupplierData = $obj->view_all_by_cond("vw_purchase", "`supplier` = '$supplierId' AND MONTH(entry_date)='$dateMonth' and YEAR
    (entry_date)='$dateYear' order by entry_date");
    $supplierPaymentData = $obj->view_all_by_cond("tbl_account", "`cus_or_sup_id` = '$supplierId'  AND MONTH(entry_date)='$dateMonth' and YEAR
    (entry_date)='$dateYear' order by entry_date");
} else {
    $date = date('Y-m-d');
    $allSupplierData = $obj->view_all_by_cond("vw_purchase", "`supplier` = '$supplierId' AND MONTH(entry_date)=MONTH('$date') and YEAR
...
<div class="col-md-12 bg-teal-800" style="margin-top:20px;">
    <div class="col-md-8">
        <h4><strong>View Ledger Supplier name <?php echo $supplierName ?> of
    ...
    <div class="col-md-4" style="padding-top:5px;">
        <?php if ($ty == 'SA') { ?>
            <button type="submit" class="btn btn-primary btn-sm pull-right"
                    onclick="printDiv('div_all')">Print Supplier Ledger
            </button>
        <?php } ?>
    </div>
</div>
<div class="col-md-12 bg-teal" style="margin-bottom: 15px;padding:5px 0;">
    <div class="col-md-6"><p>Mobile : <?php echo $supplierPersonalData['supplier_mobile_no'] ?></p></div>
    <div class="col-md-6"><p>Address : <?php echo $supplierPersonalData['supplier_address'] ?></p></div>
</div>

<div class="col-md-12 bg-slate-800" style="margin-bottom: 20px;">
    <form action="" method="POST">
        <div class="col-md-5">
            <div class="form-group">
                <label class="control-label col-sm-4" style="padding-top:5px" for="dateMonth">Select Month</label>
                <div class="col-sm-8">
                    <select class="form-control" required="required" name="dateMonth"
                            id="status">
                        <option></option>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover table-striped">
                ...
                <tr>
                    ...
                    <th class="text-center col-md-1">Bill Id</th>
                ...
                foreach ($allSupplierData as $supplier) {
                    ...
                    <tr>
                        <td class="text-center ">
                            <a type="button" target="_blank" href="pdf/bill.php?billId=<?php echo $bill_id ?>"
                        ...
                        <td class="text-center"><?php echo isset($supplier['total_price']) ? number_format($supplier['total_price']) . ' tk.' : NULL; ?></td>
                        <td class="text-center"><?php echo isset($supplier['payment_recieved']) ? number_format($supplier['payment_recieved']) . ' TK.' : NULL; ?></td>
                        <td class="text-center"><?php echo isset($supplier['due_to_company']) ? number_format($supplier['due_to_company']) . ' TK.' : NULL; ?></td>
...
<div class="col-md-12 bg-teal-800" style="margin-top:20px; margin-bottom: 15px;">
    ...
    <div class="col-md-4" style="padding-top:5px;">
        <?php if ($ty == 'SA') { ?>
            <button type="submit" class="btn btn-primary btn-sm pull-right"
                    onclick="printDiv('print_supplier_ledger')">Print Supplier Ledger
            </button>
        <?php } ?>
    </div>
...
<div id="print_supplier_ledger" class="row" style="font-size: 14px;">
    ...
    <?php if ($supplierPaymentData) { ?>
        <div class="col-md-12">
            <h5><strong>Supplier name <?php echo $supplierName ?> of
                    <?php if (isset($_POST['search'])) {
                        echo date("M, Y", strtotime("1." . $_POST['dateMonth'] . "." . $_POST['dateYear'] . ""));
                    } else {
                        echo 'This Months';
                ...
                <table class="table table-bordered table-hover table-striped" id="datatable-payment">
                    ...
                    foreach ($supplierPaymentData as $supPay) {
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <?php echo !empty($supPay['purchase_or_sell_id']) ? str_replace('p_', '', $supPay['purchase_or_sell_id']) : 'N/A' ?>
                            </td>
                            <td class="text-center">
                                <?php
                                if ($supPay['acc_type'] == 2) {
                                    echo 'Payment given with Purchase / Invoice';
                                } else if ($supPay['acc_type'] == 3) {
                                    echo 'Payment Received with Sell /Bill';
                                } else if ($supPay['acc_type'] == 5) {
                                    echo 'Customer Individual Payment';
                                } else if ($supPay['acc_type'] == 6) {
                                    echo 'Supplier Individual Payment';
                                } else if ($supPay['acc_type'] == 9) {
                                    echo 'Supplier Opening Advance';
                                } else if ($supPay['acc_type'] == 10) {
                                    echo 'Supplier Opening Due';
                                } else {
                                    echo 'N/A';
                                }
                                ?>
                            ...
                            <td class="text-center">
                                <small><?php echo isset($supPay['acc_description']) ? $supPay['acc_description'] : NULL; ?></small>
                            </td>
                            <td class="text-center">
                                <small><?php echo isset($supPay['acc_amount']) ? number_format($supPay['acc_amount']) . ' tk' : NULL;
                                    ?></small>
                            </td>
                            <td>
                                <?php
                                if(empty($supPay['purchase_or_sell_id'])){ ?>
                                    <a type="button"
                                       data-name = "<?php echo $supplierName; ?>"
                                       data-supplier = "<?php   ?>"
                                       data-accid = "<?php echo $supPay['acc_id'] ?>"
                                ...
                                <?php } else{ echo '<a href="?q=edit_purchase&billId='.trim($supPay['purchase_or_sell_id'],'p_').'" type="button" class="btn bg-teal btn-success btn-xs"> Adjust</a>';} ?>
                            </td>
                        </tr>
                        <?php
                    }
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover table-striped">
                ...
                <tr>
                    ...
                    <td class="text-center bg-success">
                        <strong><?php echo number_format($obj->get_sum_data('tbl_account','acc_amount', "acc_type = 11"))?> tk</strong>
                    </td>
                </tr>
                <tr>
