...
$accountDetailsCash = $sell_details = $obj->view_all_by_cond("vw_accounts_with_acc_head_other_income", "payment_method=1 AND entry_date BETWEEN '$startDate' and '$endDate' ORDER BY `vw_accounts_with_acc_head_other_income`.`entry_date` ASC");

$accountDetailsBank = $sell_details = $obj->view_all_by_cond("vw_accounts_with_acc_head_other_income", "payment_method=0 AND entry_date BETWEEN '$startDate' and '$endDate' ORDER BY `vw_accounts_with_acc_head_other_income`.`entry_date` ASC");
?>
...
<div class="col-md-12 bg-grey-800" style="margin-top:20px; margin-bottom: 15px;">
    <div class="col-md-6">
        <h4><strong>Cash Accounts Statement Between <?php echo date("d-M-Y", strtotime($startDate)) . " to " . date("d-M-Y", strtotime($endDate)); ?></strong>
        </h4>
    </div>

    <div class="col-md-6" style="padding-top:5px;">
        <a href="?q=print_account_statement" target="_blank" class="btn btn-primary btn-sm pull-right">Print Statement
        </a>
    </div>
</div>
...
<div class="row" id="acc_statment_print">
    <div class="col-md-12" style="font-size:12px;">
        <h4 class="text-center"><strong>Cash Accounts Statement Between <?php echo date("d-M-Y", strtotime($startDate)) . " to " . date("d-M-Y", strtotime($endDate)); ?></strong>
        </h4>
        <table class="table table-responsive table-bordered table-hover table-striped">
            ...
            foreach ($accountDetailsCash as $account) {
                $debit = 0;
                $credit = 0;
                if ($account['acc_type']==7||$account['acc_type']==8||$account['acc_type']==9||$account['acc_type']==10){
                    continue;
                }
        ...
        <hr>
        <h4 class="text-center"><strong>Bank Statement Between <?php echo date("d-M-Y", strtotime($startDate)) . " to " . date("d-M-Y", strtotime($endDate)); ?></strong></h4>
        <table class="table table-responsive table-bordered table-hover table-striped">
            <thead>
            <tr class="bg-slate-800">
                <th class="col-md-1 text-center">#</th>
