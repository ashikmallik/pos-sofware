...
if (isset($_POST['add_purchase'])) {
    ...
    foreach ($product_id as $id) { // Configure array for DB insert
        $itm_price = !empty($price[$i]) ? $price[$i] : 0;
        $item_qty = !empty($qty[$i]) ? $qty[$i] : 0;
        $extra_price = !empty($extra_price[$i]) ? $extra_price[$i] : 0; // Handle extra price
        $total_qty += $item_qty;

        $item_total = ($itm_price * $item_qty) + $extra_price; // Base total with extra price

        if (isset($commision) && !empty($commision)) {
            $item_total -= ($item_total * $commision) / 100; // Apply commission adjustment
        }
    ...
    if (isset($less) && !empty($less)) {
        $total_amount -= $less; // Apply less adjustment
    }
    ...
    $form_purchase_add = array(
        'supplier' => $sup_id,
        'total_price' => $total_amount, // Final payment amount after all adjustments
        'less_amount' => $less,
        'discount' => $commision,
...
<div class="row" style="padding:10px; font-size: 12px;">
    ...
    <thead>
        <tr>
            ...
            <th class="col-md-2 text-center">Extra Price</th>
    ...
    $('input#total_price').val(totalPrice - parcent_amount - less_amount);
    $('input#total_price_amount').val(totalPrice - parcent_amount - less_amount);
    return totalPrice - parcent_amount - less_amount;
}

// Add this function to calculate each row's total including extra price
// Update total price and total field on keyup for qty, price, and extra_price
$("#orderTable").on('keyup', 'input#qty, input#price, input#extra_price', function () {
    var price = $(this).closest('tr').find('input#price').val() || 0;
    var qty = $(this).closest('tr').find('input#qty').val() || 0;
    var extraPrice = $(this).closest('tr').find('input#extra_price').val() || 0;

    var total = (price * qty) + parseFloat(extraPrice); // Calculate row total with extra price
    $(this).closest('tr').find('input#total').val(total);

    $('input#total_price').val(totalPriceCalculate()); // Update total price
    $('input#total_quantity').val(totalQtyCalculate()); // Update total quantity
});
...
function totalPriceCalculate() {
    ...
    $('input#total_price').val(finalTotal); // Update final total price
    $('input#total_price_amount').val(finalTotal); // Set total price for form submission

    return finalTotal; // Return final calculated price
}
...
$('select[name="item_name"]').on('change', function (e) { // add new row when new item selected
    ...
    if (itemId) {
        addRow('<td>' + crow + '</td><td><input type="text" value="' + itemName + '" class="form-control"></td>' +
            ...
            '<td><input style="width:90%;" type="text" onkeypress="return numbersOnly(event)"  id="price" name="price[]" value="' + itemPrice + '" class="form-control"></td>' +
            '<td><input style="width:90%;" type="text" onkeypress="return numbersOnly(event)"  id="extra_price" name="extra_price[]" placeholder="Extra Price" class="form-control"></td>' +
