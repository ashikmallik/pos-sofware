...
$total_amount = 0;
$due = 0;
$loan_give_type = 13; // for accounts
if (isset($_POST['add_loan'])) {
    ...
    $form_tbl_accounts = array(
        'acc_description' => "Person Take loan " . $loan . "tk From Company  " . $person_name . ". ".$description,
        'acc_amount' => $loan,
        'acc_type' => $loan_give_type,
        'purchase_or_sell_id' => 'e_' . $person_id,
    ...
    if ($obj->insert_by_condition("tbl_person_loan", $form_tbl_person, " ")) {

        $obj->notificationStore('Loan give to ' . $personData['person_name'] . ' Successfully', 'success');
        echo '<script> window.location.href = window.location.href; </script>';

    } else {
        $obj->notificationStore('Loan give to Person Failed');
        echo '<script> window.location.href = window.location.href; </script>';
    }
}
    ...
    function numbersOnly(e) // Numeric Validation
    {
        ...
        if (unicode != 8) {
            if ((unicode < 2534 || unicode > 2543) && (unicode < 46 || unicode > 57)) {
                return false;
            } else if (unicode == 47) {return false;}
        }
    }
...
<div class="row">
    <div class="col-md-12 bg-teal-800 text-center">
        <h4>Welcome to Assign Loan To Person</h4>
    </div>
</div>
<div class="row">
...
<div class="row" style="font-size: 12px;">
    <form id="purchase_form" class="form-horizontal" role="form" enctype="multipart/form-data" method="post">
        ...
        <div class="col-md-8 col-md-offset-1">
            <div class="form-group">
                <label class="control-label col-sm-4" for="person_id">Loan Amount (Give) :</label>
                <div class="col-sm-8">
