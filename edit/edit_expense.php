...
$expenseId = (isset($_GET['token']) && !empty($_GET['token'])) ? $_GET['token'] : null;

$expenseData = $obj->details_by_cond('tbl_account', "acc_id = '$expenseId'");
...
if (isset ($_POST['editExpense'])) {
    ...
    $tbl_accounts_update = $obj->Update_data("tbl_account", $form_tbl_accounts, "acc_id = '$expenseId'");

    if ($tbl_accounts_update) {
        ?>
        <script>
            window.location  = "?q=edit_expense&token=<?php echo $expenseId; ?>";
        </script>
        <?php
    } else { // $paid_amount <= $total_price
        $notification = '<div class="alert alert-danger">Update Failed</div>';
    }
        ...
        <div class="col-md-7 col-md-offset-2">
                ...
                <div class="col-sm-8">
                    <select class="form-control" name="acc_id" id="status">
                        ...
                        foreach ($obj->view_all_by_cond("tbl_ac_head_other_income", "ac_head_or_other_income = 1") as
                            ...
                            <?php echo ($acc_head['acc_id'] == $expenseData['acc_head']) ? 'selected' : '' ;?>><?php echo
                                isset($acc_head['acc_name']) ? $acc_head['acc_name'] : NULL;
                                ?></option>
                            <?php
                        }
                        ?>
            ...
            <div class="row">
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input type="text" value = "<?php echo isset($expenseData['acc_amount']) ?
                            $expenseData['acc_amount'] : null ;  ?>" onkeypress="return numbersOnly(event)"
                               required="required" name="amount" class="form-control"
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <textarea class="form-control" name="details" id="ResponsiveDetelis" rows="4"><?php echo isset($expenseData['acc_amount']) ?
                                $expenseData['acc_description'] : null ;  ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="row margin_top_10px" style="margin-bottom:20px;">
                <div class="col-md-2 col-md-offset-7">
                    <div class="text-center">
                        <button type="submit" class="btn btn-block btn-success" name="editExpense">Submit</button>
                    </div>
                </div>
            </div>
        </div>
