...
if (isset($_POST['search'])) {
    extract($_POST);
    $monthlySell = $obj->view_all_by_cond("vw_sell_daily", "MONTH(entry_date)='$dateMonth' AND YEAR
    ...
    $date = $_GET['dtoken'];
    $monthlySell = $obj->view_all_by_cond("vw_sell_daily", "MONTH(entry_date)=MONTH('$date') AND YEAR
    (entry_date)= YEAR('$date') order by entry_date");

    $print = 'action=monthly&monthDate='.$date.'';
    $header = date('M Y', strtotime($date));
} else {
    $monthlySell = $obj->view_all_by_cond("vw_sell_daily", "month(entry_date) = month(CURDATE()) AND YEAR
...
<div class="col-md-12 bg-slate-800" style="margin-top:20px; margin-bottom: 15px;">
    <div class="col-md-6">
        <h4><strong>View All
                ...
                }else{echo 'This Months';} ?> Sales List </strong></h4>
    </div>
    <div class="col-md-6" style="padding-top:5px;">
        <?php if ($ty == 'SA') { ?>
            <a href="?q=print_monthly_yearly_sell&<?php echo $print ?>" target="_blank" class="btn btn-primary btn-sm pull-right">Print Sell
                List</a>
        <?php } ?>
    </div>
</div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover table-striped" id="datatable">
                ...
                foreach ($monthlySell as $sell) {
                    ...
                    <tr>
                        <td><?php echo $i; ?></td>
                        <td class="text-center"><a href="?q=today_sale_report&dtoken=<?php echo $sell['entry_date'];
                            ?>"> <?php echo isset($sell['entry_date']) ? date('d-M-y', strtotime($sell['entry_date'])) : NULL; ?></a></td>
                        <td class="text-right"><?php echo isset($sell['total_qty']) ? number_format($sell['total_qty']) . ' pcs' : NULL; ?></td>
                        <td class="text-right"><?php echo isset($sell['total_price']) ? number_format($sell['total_price']) . ' TK.' : NULL; ?></td>
