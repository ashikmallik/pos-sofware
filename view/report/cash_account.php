...
$accountDetailsBank = $sell_details = $obj->view_all_by_cond("vw_accounts_with_acc_head_other_income", "acc_type=25 AND entry_date BETWEEN '$startDate' and '$endDate' ORDER BY `vw_accounts_with_acc_head_other_income`.`entry_date` ASC");
?>
...
<div class="col-md-12 bg-grey-800" style="margin-top:20px; margin-bottom: 15px;">
    <div class="col-md-6">
        <h4><strong>Cash Statement Between <?php echo date("d-M-Y", strtotime($startDate)) . " to " . date("d-M-Y", strtotime($endDate)); ?></strong>
        </h4>
    </div>
                    ...
                    <td class="">
                        ...
                        if ($account['acc_head'] != '0') {
                           ...
                           // echo $account['acc_type'];
                            if ($account['acc_type']!=$bankWithdrow && $account['acc_type']!=$bankDeposit) {
    ...
    $(document).ready(function() {
        ...
        $('#datatable-btn').DataTable( {
            ...
            buttons: [
                {
                    extend: 'print',
                    text: 'Print Cash Statement',
                    footer: true,
                    title: function () {
                        return "Cash Transection <?php echo date("d-M-Y", strtotime($startDate)) . " to " . date("d-M-Y", strtotime($endDate)); ?>"
                    },
                    exportOptions: {
