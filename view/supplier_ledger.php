...
if (isset($_POST['search'])) {
    ...
    $openiongblnacea = $obj->get_sum_data('tbl_account','acc_amount',"acc_type=9 AND cus_or_sup_id='$supplierId' AND entry_date BETWEEN '$startDate' and '$endDate' order by entry_date");

    $openiongblnaced = $obj->get_sum_data('tbl_account','acc_amount',"acc_type=10 AND cus_or_sup_id='$supplierId' AND entry_date BETWEEN '$startDate' and '$endDate' order by entry_date");
    // var_dump($openiongblnace);
} else {
    $date = date('Y-m-d');
    ...
    $supplierPaymentData = $obj->view_all_by_cond("tbl_account", "`cus_or_sup_id` = '$supplierId' AND MONTH(entry_date)=MONTH('$date') and YEAR(entry_date)= YEAR('$date') order by entry_date");
    ...
    $openiongblnacea = $obj->get_sum_data('tbl_account','acc_amount',"acc_type=9 AND cus_or_sup_id='$supplierId' AND MONTH(entry_date)=MONTH('$date') and YEAR(entry_date)= YEAR('$date')");

    $openiongblnaced = $obj->get_sum_data('tbl_account','acc_amount',"acc_type=10 AND cus_or_sup_id='$supplierId' AND MONTH(entry_date)=MONTH('$date') and YEAR(entry_date)= YEAR('$date')");
  
  
}
/*$balance_bd_allSupplierData = $obj->view_all_by_cond("vw_purchase", "supplier='$supplierId' AND entry_date < '$startDate'");
                    $total_qty_bd = 0;
                    $total_price_bd = 0;
                    $total_payment_bd = 0;
                    $total_due_bd = 0;
                    foreach ($balance_bd_allSupplierData as $supplier) {
                        $i++;
                        $total_qty_bd = $supplier['total_qty'] + $total_qty_bd;
                        $total_price_bd = $supplier['total_price'] + $total_price_bd;
                        $total_payment_bd = $supplier['payment_recieved'] + $total_payment_bd;
                        $total_due_bd += $supplier['due_to_company'];
                        ?>
                    <?php } */

$balance_bd = $obj->view_all_by_cond("tbl_account", "cus_or_sup_id='$supplierId' AND entry_date < '$startDate'");
$balance_of_bd = 0;
$total_debit_bd = 0;
$total_credit_bd = 0;
foreach ($balance_bd as $supPay) {
                            $sl++;
                            $debit_bd = 0;
                            $credit_bd = 0;
                            ...
                            ) {$debit_bd = $supPay['acc_amount'];
                                $balance_of_bd += $debit_bd;
                                $total_debit_bd += $debit_bd;

                            } else {
                                $credit_bd = $supPay['acc_amount'];
...
if (isset($_POST['adjust'])) {
    ...
    if ($adjustment_amount) {?>
        <script>
            window.location = 'index.php?q=supplier_ledger&supplierId=<?= $_GET["supplierId"]?>';
        </script>
        <?php
    }
}
...
<div id="div_all" class="row" style="font-size: 12px;">
    <div class="col-md-10" style="margin-bottom: 10px;">
        <h3 style="font-weight: bold; color: gray;">Terget Seal : <?php echo number_format($supplierPersonalData['target'],2) ?> TK</h3>
    </div>
    <div class="col-md-12">
            ...
            <table class="table table-bordered table-hover table-striped" id="datatable-btn">
                ...
                <tbody>
                    ...
                    foreach ($allSupplierData as $supplier) {
                        ...
                        $total_comm += $supplier['total_comission_earn'];
                        ...
                        <tr>
                            ...
                            <td class="text-right"><?php echo isset($supplier['total_comission_earn']) ? number_format($supplier['total_comission_earn'], 2) . ' tk.' : NULL; ?></td>
...
<div id="print_supplier_ledger" class="row" style="font-size: 14px;">
    <?php 
    $sellItemData = $obj->view_all_by_cond('vw_purchase_item', "bill_id = 73");
                      ...
                      //  $balance = 0;
                        ...
                        foreach ($supplierPaymentData as $supPay) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    if($supPay['acc_type'] == '5' || $supPay['acc_type'] == '6'){ ?>
                                        <button type="button" data-amount="<?= $supPay['acc_amount']?>" data-accid = "<?=$supPay['acc_id']?>" data-toggle="modal"data-target="#adjustment-modal" class="btn bg-teal btn-success btn-xs">Adjust
                                        </button>
                                    <?php } ?>
                                </td>
                            </tr>
            ...
            <div class="row" style="padding:10px; font-size: 12px;">
                <form class="form-horizontal" role="form" enctype="multipart/form-data" method="post">
                    <div class="col-md-10  col-md-offset-1" style="font-size: 12px;">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <label class="control-label col-sm-4" for="adjustment_type" id="adjustment_type">Adjustment Type:</label>
                            <div class="col-md-8">
