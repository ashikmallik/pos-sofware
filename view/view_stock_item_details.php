                ...
                <table class="table table-striped table-bordered">
                    <thead class="bg-grey-600">
                        <tr style="font-size:12px;">
                            ...
                            <th class="col-md-2 text-center" colspan="2">Delivery</th>
                    ...
                    foreach ($purchaseSaleItemData as $stockItem) {
                        ...
                        $sell_id = trim($stockItem['bill_or_sell_id'],'s_');
                        $sell_id_p = preg_replace('/[0-9]+/', '', $stockItem['bill_or_sell_id']);
                        if($sell_id_p=='s_'){
                            $stockItem['entry_date'];
                            $sell_id = intval($sell_id);
                            $total_delevered  = $obj->get_sum_data("tbl_delivery_item","qty","sell_no=$sell_id AND product_id=$stockId AND delivery_date='".$stockItem['entry_date']."'");
                        }
                        //print_r($total_delevered);
                        ?>
                        <tr>
                            ...
                            <?php } else{ ?>
                                <td colspan="3"></td>
                                <td class="text-center"><?php $total_chalan += $total_delevered; echo $total_delevered; ?></td>
                            ...
                            <td class="text-center"><strong><?php $present_stock = $total_receipt - $total_delevered; echo $present_stock; ?></strong></td>
